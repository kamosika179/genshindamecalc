public class CalcDame extends Status{
    ...
    void print_normal_damege(){
        ...
        for(int i = 0;i < character.normal_talent_magnification.length;i++){
            double damage = ((character.atk + weapon.atk)*(1 + this.atk_per*0.01) + this.atk) * character.normal_talent_magnification[i] *0.01 * (1 + this.crit_dame*0.01) * (1 + this.phy_buff*0.01) * 1 * ((100 + character.lv)/((1 - def_debuff)*(100 + enemy.lv) + (100 + character.lv)))*((1 -(enemy.debuff_resist*0.01-resist_debuff)));
            System.out.println(character.normal_talent_name[i + 2]+":"+damage);
        }
    }
}
